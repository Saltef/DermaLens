...
- **MobileNet checkpoint-selection diagnostic:** if the same MobileNet training histories are reselected post hoc by validation accuracy instead of macro recall, MobileNet reaches 52.4% +/- 5.2 accuracy and 25.1% +/- 3.3 macro recall. This is optimistic because it chooses from the evaluation-fold history. The conservative envelope is therefore >= +17.3 accuracy points and >= +5.6 macro-recall points for Derm Foundation against the best MobileNet policy for each metric separately. See `models/grouped_scin_mobilenet_checkpoint_selection_diagnostic.json`.
...
- **Generic frozen-encoder control:** ConvNeXt-Tiny ImageNet embeddings under the same grouped/nested probe protocol reached 53.1% +/- 4.1 accuracy and 23.1% +/- 3.3 macro recall. Derm Foundation remains ahead by +16.7 accuracy points and +11.6 macro-recall points, but the comparison still bundles dermatology pretraining with much larger BiT-101x3 capacity and 448px input resolution. See `models/grouped_scin_convnext_tiny_embedding_12seed_metrics.json`.
...
This uses Google's `google/derm-foundation` embedding model as a frozen representation, trains a class-balanced linear probe, selects C on a nested grouped calibration split, and evaluates once on the held-out grouped fold. The existing 12-seed artifact was produced before embedding source was recorded, so it is marked `legacy_cache_source_unrecorded`; future reruns can use `--embedding-source local-model` to bypass Google's SCIN precomputed embedding file.
...
I also checked whether the accuracy lift was mainly caused by MobileNet's macro-recall checkpoint selection. The diagnostic answer is no, though the gap shrinks: reselecting MobileNet epochs post hoc for validation accuracy raises MobileNet to 52.4% accuracy. To avoid mixing MobileNet policies, the conservative statement is: Derm Foundation is at least +17.3 points better on accuracy and at least +5.6 points better on macro recall against the most favorable MobileNet policy for each metric separately. This diagnostic is not a clean estimate because it uses the evaluation history for epoch choice; it is included only to make the comparison harder to overstate.

I then ran a generic frozen-encoder control with ConvNeXt-Tiny ImageNet embeddings and the same nested grouped linear-probe protocol. ConvNeXt reached 53.1% accuracy and 23.1% macro recall, so Derm Foundation still leads. The attribution claim is now narrower: this supports a stronger dermatology-specific representation result on SCIN, but it does not isolate dermatology pretraining from model scale and input resolution. Derm Foundation is a BiT-101x3-style 6144-dimensional, 448px representation; MobileNetV3-Small is a compact 224px deployed model. Google's own documentation also presents SCIN as a public downstream linear-classifier use case for Derm Foundation, so this remains a vendor-home downstream benchmark rather than external validation.
